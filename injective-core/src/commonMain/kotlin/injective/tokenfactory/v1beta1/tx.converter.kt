...
public expect object MsgSetDenomMetadataConverter : ProtobufConverter<MsgSetDenomMetadata>

public fun MsgSetDenomMetadata.toAny(): Any = Any(MsgSetDenomMetadata.TYPE_URL,
    with(MsgSetDenomMetadataConverter) { toByteArray() })

public fun Any.parse(converter: ProtobufConverter<MsgSetDenomMetadata>): MsgSetDenomMetadata {
  if (typeUrl != MsgSetDenomMetadata.TYPE_URL) throw
      IllegalStateException("Please check the type_url")
  return value.parseProtobuf(converter)
}

public expect object MsgSetDenomMetadataResponseConverter :
    ProtobufConverter<MsgSetDenomMetadataResponse>

public fun MsgSetDenomMetadataResponse.toAny(): Any = Any(MsgSetDenomMetadataResponse.TYPE_URL,
    with(MsgSetDenomMetadataResponseConverter) { toByteArray() })

public fun Any.parse(converter: ProtobufConverter<MsgSetDenomMetadataResponse>):
    MsgSetDenomMetadataResponse {
  if (typeUrl != MsgSetDenomMetadataResponse.TYPE_URL) throw
      IllegalStateException("Please check the type_url")
  return value.parseProtobuf(converter)
}
